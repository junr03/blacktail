...
Review the selected profile's macOS username, Git identity, public SSH keys, SSH identity filenames, and remote usernames before the first activation. To add another Mac, create another profile under `hosts/` and register it in `hostProfiles` in `flake.nix`; do not put machine-specific identity into the shared modules.

Private keys are deliberately not stored or generated by this repository. The selected profile expects these files:
